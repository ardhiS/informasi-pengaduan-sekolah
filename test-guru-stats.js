// Test stats endpoint specifically for guru role
const testGuruStatsAccess = async () => {
  try {
    console.log('🔍 Testing guru access to stats endpoint...');

    // Login as guru with known credentials
    const authResponse = await fetch('http://localhost:5000/authentications', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username: 'guru01', password: 'guru123' }),
    });

    if (!authResponse.ok) {
      console.log('❌ Guru auth failed:', authResponse.status);
    ...
    console.log('✅ Guru token obtained successfully');
    ...
    if (complaintsResponse.ok) {
      ...
      console.log('✅ Complaints accessible for guru');
