...
/**
 * Mendapatkan daftar semua pengaduan dengan filter dan paginasi.
 * @param {object} params - { status, category, priority, startDate, endDate, page, limit }
 * @returns {Promise<object>}
 */
async function getComplaints(params = {}) {
  const queryString = buildQueryString(params);
  const response = await fetchWithToken(
    `${BASE_URL}/api/complaints?${queryString}`
  );
  return response.json();
}

/**
 * Memperbarui pengaduan berdasarkan ID.
 * @param {string} id - Complaint ID
 * @param {object} complaintData - { title, description, category, priority }
 * @returns {Promise<object>}
 */
...
/**
 * Menyetujui pengaduan.
 * @param {string} id - Complaint ID
 * @param {object} payload - { admin_notes }
 * @returns {Promise<object>}
 */
async function approveComplaint(id, payload) {
  const response = await fetchWithToken(
 ...
 * @returns {Promise<object>}
 */
async function rejectComplaint(id, payload) {
  const response = await fetchWithToken(
